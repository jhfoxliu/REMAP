...
def mapping(read1, read2=None, out_prefix=None, p=4):
    os.system("{hisat2} --no-temp-splicesite --no-unal -p {p} -x {index} -U {read1} -S {out_sam} ".format(
    ...
    os.system("samtools view -bS -q 60 {out_sam} > {out_bam}".format(
        out_sam = out_prefix + ".sam", 
    ...
    os.system("/home/fox/Software/bin/python /home/fox/Scripts/ReCappable-seq/Align_to_cap_starts_SE_fix_softclips.all.py {out_sorted_bam}  > {bed_out}".format(
        out_sorted_bam = out_prefix + ".sorted.bam",
